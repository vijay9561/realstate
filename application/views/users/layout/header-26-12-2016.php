...
/* Close Button */
    ...
    $('[data-popup-close]').on('click', function(e)  {
		...
		<a  href="#" class="btn btn-danger leftpostproperty mydbubttt" style="right:-39px;"  href="#" data-popup-open="popup-1"><i class="glyphicon glyphicon-briefcase"> </i> Post Free Ad
		</a>
		<?php }else{ ?>
			<a  href="#" data-popup-open="popup-1" class="btn btn-danger leftpostproperty mydbubttt" style="right:-39px;"><i class="glyphicon glyphicon-briefcase"> </i> Post Free Ad</a>
		<?php } ?>
	...
	<div class="container">
		...
		<div class="top-nav">
			...
			<!--	<li><span ><i class="glyphicon glyphicon-phone"> </i>+91 8087 245 555</span></li>-->
				<?php if($this->session->userdata('UID')){ ?>
				<li class="dropdown"><a   href="#" data-popup-open="popup-1" class="btn btn-danger mydbubttt dropdown-toggle">
 ...
 <i class="glyphicon glyphicon-user"> </i>Hi <?php echo substr($this->session->userdata('NAME'),0,10); ?><i class="fa fa-caret-down" aria-hidden="true"></i></a>
  <ul class="dropdown-menu">
	...
	<li><a style="color: #2a2727;" href="<?php echo site_url();?>users-posted-prorperty">Your Posted Ad List</a></li>
				...
				<?php }else{ ?>
					<li><a   href="#" data-popup-open="popup-1" class="btn btn-danger mydbubttt"><i class="glyphicon glyphicon-briefcase"> </i> Post Free Ad</a></li>
	...
	<div class="row">
       	<?php   $viewdata=$this->db->query("select *from main_category where mid IN(1,3,4,5,6,7) order by mid asc")->result(); foreach( $viewdata as $rows) { ?>
				 ...
				 <?php if($rows->mid==1){ ?>
				  ...
				  <div class="col-xs-6 col-md-2 mydivshover"  style="color:#000000;text-align:center;margin-bottom: 20px;padding-top: 10px;">
				
				  <img class="imageproperty img-circle" src="<?php echo base_url(); ?>adminassets/upload/category/<?php echo $rows->category_images ?>"  />
						
							<h6 style="text-align:center;padding-top: 10px;padding-bottom: 10px;"><?php echo  ($rows->category_name); ?></h6>
				  </div></a><?php }else{ ?>
				   <a href="<?php echo site_url() ?>otherpostform?category=<?php echo $rows->mid; ?>">
				   <div class="col-xs-6 col-md-2 mydivshover"  style="color:#000000;text-align:center;padding-top: 10px;">
				...
				<div class="row">
				  	<?php   $viewdata2=$this->db->query("select *from main_category where mid IN(14) order by mid asc")->result(); foreach( $viewdata2 as $rows) { ?>
				   <a href="<?php echo site_url() ?>otherpostform?category=<?php echo $rows->mid; ?>">
				 <div class="col-xs-6 col-md-2 mydivshover"  style="color:#000000;text-align:center;padding-top: 10px;">
	...
	<div class="row">
       	<?php   $viewdata=$this->db->query("select *from main_category where mid IN(1,3,4,5,6,7)  and status='active' order by mid asc")->result(); foreach( $viewdata as $rows) { ?>
				...
				<div class="row">
				  	<?php   $viewdata2=$this->db->query("select *from main_category where mid IN(14)  and status='active' order by mid asc")->result(); foreach( $viewdata2 as $rows) { ?>
				   <a href="<?php echo site_url() ?>login?category=<?php echo $rows->mid; ?>">
				 <div class="col-xs-6 col-md-2 mydivshover"  style="color:#000000;text-align:center;padding-top: 10px;">
...
<div class="header-right">
		<div class=" banner">
			 <div class="slider">
			    <div class="callbacks_container slidercolbacks">
