...
function regiterusers(){
	...
	function duplicatecompanyname(){
	if($('#name').val()==''){}else{
		var email=document.getElementById('eamil').value;
		var postTo = '<?php echo site_url(); ?>Users_Controller/duplicateemails';
		var data = { email: email};
		jQuery.post(postTo, data,
		function(data) {
			if(data==1){
				$('#eamilr').html(" ");
			}else{ $('#eamilr').html(email +"This Email ID Allready Registered");
                   $('#eamil').val(' ');
				 }
		...
		<div class="login-top">
				...
				<div>
					...
					<div class="form-group">
						...
						<div class="form-group">
						<input type="text"   placeholder="Email Adress" id="eamil" name="eamil" class="form-control" required="" maxlength="40" onChange="duplicatecompanyname()" ><span id="eamilr" style="color:red;"></span>
						</div>
						<div class="form-group">
