...
}else if(($('#property_subcategory').val()=='21') || ($('#property_subcategory').val()=='23') || ($('#property_subcategory').val()=='25') || ($('#property_subcategory').val()=='92') || ($('#property_subcategory').val()=='330')) {
$("#yearhide").hide();
$("#kmhide").hide();
...
function propertypost(){
	...
	if(middd==16){
		...
		//	$("#loading").show();	
			...
			$.ajax({   
				url: "<?php echo site_url(); ?>Admin_Controller/update_my_other_property",
				type: "POST",
				data : formData,
				async: true,
				cache: false,
				...
				success: function(data){
					if(data==1){
						window.location='<?php echo site_url();?>view_property_other';
						return false; 
					}
					else {
					$('#loading').hide();
...
<div class="container-fluid">
  ...
  <div class="row">
    <div class="col-lg-12">
      <h1 class="page-header">Update Other Services</h1>
      <ol class="breadcrumb">
        <li> <i class="fa fa-dashboard"></i> <a href="<?php echo site_url(); ?>admin">Dashboard</a> </li>
        <li class="active"> <i class="fa fa-home"></i> Update Other Services</li>
      </ol>
    </div>
  </div>
  <!-- /.row -->
  <div class="row">
    <div class="col-lg-12">
       <div class="panel panel-primary">
              <div class="panel-heading">Update Other Services</div>
	  ...
	  <div class="form-info">
          ...
          <span id="delartyper" style="color:red;"></span> </div>
				...
				<select type="text" class="form-control" id="car_name" onchange="car_namer();" name="car_name">
				<option value="<?php echo $viewdata[0]->car_name; ?>"><?php echo $viewdata[0]->car_name; ?></option>
					<?php $brandname1=$this->db->query("select tempo_name from tempo_master")->result(); 
	...
	<?php $loan_name=$this->db->query("select loan_name from loan_master")->result(); foreach($loan_name as $loan_name1){ if($viewdata[0]->loan_type!= $loan_name1->loan_name){ ?>
		...
		<?php } ?>
		  ...
		  <div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-md-6">
				<label>Parking Area (Sq.ft)</label>
				<input type="text" name="parking_squate_foot" id="parking_squate_foot"  onkeypress="return isNumber(event)"  autocomplete="off" class="form-control" onkeypress="return isNumber(event)" autocomplete="off"  />
				</div>
				<div class="col-md-6">
				<label>Praking Capacity</label>
				<input  type="text" name="parking_bike_capcity" id="parking_bike_capcity"  onkeypress="return isNumber(event)"  autocomplete="off" class="form-control" onkeypress="return isNumber(event)" autocomplete="off"  />
				</div></div></div>
				<?php } ?>
	<?php } ?>
