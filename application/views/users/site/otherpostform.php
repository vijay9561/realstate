...
<?php if($category[0]->category_name=='Electrisation') {?>
$(document).ready(function(){
$("#pricehiding").css('display','none'); 
...
$('#parking_type_tow').click(function() {
  ...
  $('#openparkingtypw').css('display','none');
	...
	} else
	{
	 ...
	 function termsandcondtionr(){
	if(document.getElementById("termsandcondtion").checked==true){ $("#termsandcondtionr").html(" "); }else{
...
function sub_sub_categoryr(){if($('#sub_sub_category').val()==''){}else{ $('#sub_sub_categoryr').html(' '); }}
...
function propertypost(){
		...
		var middd=document.getElementById('mainhiddencategory').value.trim();	
				...
				if(photo==''){
				$("#lblError").html('Please upload images');
				 $( "#photo" ).focus();
				return false;
				}
		   ...
		   var car_name=document.getElementById('car_name').value.trim();
				...
				if(car_year==''){
				$("#car_yearr").html('Please enter car purchase year');
				$("#car_year" ).focus();
				return false;
				}
			...
			var  area=document.getElementById('area').value.trim();
		     var car_mdoel_name=document.getElementById('car_mdoel_name').value.trim();
			   var water_storage_capcity=document.getElementById('water_storage_capcity').value.trim();
				...
				if (!(car_mdoel_name.match(digitno))) {
				$("#car_mdoel_namer").html("Please enter only digit");	
				$("#car_mdoel_namer" ).focus();
				return false;
				}
				if(property_subcategory==25 || property_subcategory==24 ){
			...
			if(middd==14){
			 ...
			 if($('input[type=checkbox]:checked').length == 0)
				...
				return false;
	              ...
	              if($('input[type=checkbox]:checked').length == 0)
                   {
			   	$("#single_room_typer").html("Please select hall facility");	
				return false;
				 }
	...
	return false;
		...
		if(property_owner_name==''){
				...
				if(document.getElementById("termsandcondtion").checked==true){	
	            }else{
	             $('#termsandcondtionr').html("please check terms and condtion");
				 $("#termsandcondtion").focus();
	             return false;
	             }
		//	$("#loading").show();	
			$("#loaderimages").show();
			...
			$.ajax({   
				url: "<?php echo site_url(); ?>Users_Controller/otherproperty_category",
				type: "POST",
				data : formData,
				async: true,
				cache: false,
...
<?php  $this->session->unset_userdata('registrationsucess'); } ?>
          ...
          <label>Upload  Multiple Ad Images (upto 8 images)<b style="color:#FF0000"> *</b></label>
          <div class="container kv-main" id="dataconvasss" style="width:100%">
			...
			<p><span id="imagesizevalidation" style="color:red;"></span></p>
			<p><span style="color:red;" id="lblError"></span></p>
        </div></div>
				<div class="form-info">
				...
				<input type="text" class="form-control" id="property_category" value="<?php echo $category[0]->category_name; ?>" data-popup-open="popup-1" name="property_category" />
				...
				<div class="form-group">
				<?php $subcategory=$this->db->query("select *from subcategory where mid='".$_GET['category']."' order by category_name asc")->result(); ?>
				<label>Sub Category <b style="color:#FF0000"> *</b></label>
				<select type="text" class="form-control" id="property_subcategory" onchange="property_subcategoryr();" name="property_subcategory">
				<option value="">Select Sub Category</option>
				<?php foreach($subcategory as $rows11) { ?>
				<option value="<?php echo $rows11->scid; ?>"><?php echo $rows11->category_name; ?></option>
				<?php } ?>
				</select>
				<span id="property_subcategoryr" style="color:red;"></span>
				</div>
				
				<div class="form-group" id="hide_ad_title">
          ...
          <input type="radio" id="allmostlike1" name="allmostlike" onchange="allmostliker()" value="Almost like new" />
          <label for="radio4">Almost like new</label>
          <input type="radio"  id="allmostlike2" name="allmostlike"  onchange="allmostliker()" value="Gentaly Used" />
          <label for="radio5">Gentaly Used</label>
          <input type="radio" id="allmostlike3" name="allmostlike" onchange="allmostliker()"  value="Heavily used" />
          ...
          <input type="radio" id="allmostlike1" name="allmostlike" onchange="allmostliker()" value="Company" />
          <label for="radio4">Company</label>
          <input type="radio"  id="allmostlike2" name="allmostlike"  onchange="allmostliker()" value="Placements / Consultancy" />
          ...
          <input type="radio" id="allmostlike1" name="allmostlike" onchange="allmostliker()" value="Class tution" />
          <label for="radio4">Class tution</label>
          <input type="radio"  id="allmostlike2" name="allmostlike"  onchange="allmostliker()" value="Home tution" />
          ...
          <span id="allmostliker" style="color:red;"></span> </div>
				...
				<option value="">Select Brand Name</option>
					<?php $brandname1=$this->db->query("select name from electronic_master")->result(); foreach($brandname1 as $brandname){ ?>
				...
				<select type="text" class="form-control" id="car_name" onchange="car_namer();" name="car_name">
				<option value="">Select Brand Name</option>
				<?php $car=$this->db->query("select car_name from car_master order by car_name asc")->result(); foreach($car as $carname){ ?>
				<option value="<?php echo $carname->car_name; ?>"><?php echo $carname->car_name; ?></option>
				<?php } ?>
				</select>
				<span id="car_namer" style="color:red;"></span>
				</div>
				...
				<select type="text" class="form-control" id="car_fuel" onchange="car_fuelr();" name="car_fuel">
				...
				<select type="text" class="form-control" id="car_name" onchange="car_namer();" name="car_name">
				<option value="">Select Brand Name</option>
				<?php $bikename=$this->db->query("select bike_name from bike_master order by bike_name asc")->result(); foreach($bikename as $bikename1){ ?>
				<option value="<?php echo $bikename1->bike_name; ?>"><?php echo $bikename1->bike_name; ?></option>
				<?php } ?>
				</select>
				<span id="car_namer" style="color:red;"></span>
				</div>
				...
				<select type="text" class="form-control" id="car_name" onchange="car_namer();" name="car_name">
				<option value="">Select Brand Name</option>
				<?php $tempo=$this->db->query("select tempo_name from tempo_master order by tempo_name asc")->result(); foreach($tempo as $temponame){ ?>
				<option value="<?php echo $temponame->tempo_name; ?>"><?php echo $temponame->tempo_name; ?></option>
				<?php } ?>
				</select>
				<span id="car_namer" style="color:red;"></span>
				</div>
				...
				<?php } ?>
				<?php if($category[0]->mid==6 || $category[0]->mid==7 || $category[0]->mid==8 || $category[0]->mid==9 || $category[0]->mid==10) { ?>
				<div class="form-group" id="displaynonebyis">
          <label for="radio3">Saler Type<b style="color:#FF0000">*</b></label> &nbsp; &nbsp;
          <input type="radio" id="dealar1" name="delartype" onchange="delartyper()" value="Individual" />
          <label for="radio4">Individual</label>
          <input type="radio"  id="dealar2" name="delartype"  onchange="delartyper()" value="Dealer" />
          ...
          <input type="radio" id="hall_facility1" name="hall_facility" onchange="hall_facilityr()" value="A/C Hall" />
          <label for="radio4">A/C Hall</label>
          <input type="radio"  id="hall_facility2" name="hall_facility"  onchange="hall_facilityr()" value="Non A/C Hall" />
          ...
          <input type="radio" id="hall_decorate1" name="hall_decorate" onchange="hall_decorater()" value="With Decoration" />
          <label for="radio4">With Decoration</label>
          <input type="radio"  id="hall_decorate2" name="hall_decorate"  onchange="hall_decorater()" value="Without Decoration" />
		  ...
		  <div class="form-group">
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" id="water_storage_capcity" onchange="water_storage_capcityr();" name="water_storage_capcity">
				<span id="water_storage_capcityr" style="color:red;"></span>
				</div>
			
				<div class="form-group">
				<label>Public Capacity<b style="color:#FF0000"> *</b></label>
				<input type="text" class="form-control" id="car_mdoel_name"  onkeypress="return isNumber(event)" autocomplete="off" onchange="car_mdoel_namer();" name="car_mdoel_name">
				<span id="car_mdoel_namer" style="color:red;"></span>
				</div>
				...
				<input type="text" class="form-control" id="human_temple_distance"  autocomplete="off" onchange="human_temple_distancer();" name="human_temple_distance">
				<span id="human_temple_distancer" style="color:red;"></span>
				</div>
				<div class="form-group">
				<label for="radio3">Parking</label>
				<input type="radio" id="parking_type_one" name="parkingtypedd"  value="Yes"></label>
				<label for="radio5">Yes</label> 
				<input type="radio" value="No" id="parking_type_tow"  name="parkingtypedd">
				...
				<input  type="text" name="parking_squate_foot" id="parking_squate_foot"   autocomplete="off" class="form-control" onkeypress="return isNumber(event)" autocomplete="off"  />
				</div>
					<div class="col-md-6">
				<label>Parking Capacity</label>
				<input  type="text" name="parking_bike_capcity"  id="parking_bike_capcity"  autocomplete="off" class="form-control"  maxlength="50" autocomplete="off"  />
				</div></div>		</div>
	<?php } ?>
		<?php if($category[0]->mid=='14') { ?>
		
		<div class="form-group">
<label>Room Facility <b style="color:#FF0000"> *</b></label>
<div class="checkbox">
  <label><input type="checkbox" id="amenities1" name="amenities[]"  onclick="amenitiesr()" value="Ac">Ac</label>
</div>
<div class="checkbox">
  <label><input type="checkbox" value="Non/Ac" id="amenities2" onclick="amenitiesr()" name="amenities[]">Non/Ac</label>
</div>
<div class="checkbox">
  <label><input type="checkbox" value="Meeting Hall" id="amenities3" onclick="amenitiesr()"  name="amenities[]">Meeting Hall</label>
</div>
<div class="checkbox">
  <label><input type="checkbox" value="Conference Hall" id="amenities4" onclick="amenitiesr()" name="amenities[]">Conference Hall</label>
</div>
<div class="checkbox">
  <label><input type="checkbox" value="Luxury" id="amenities5" onclick="amenitiesr()" name="amenities[]">Luxury</label>
</div>
...
<div class="checkbox">
  <label><input type="checkbox" id="single_room_type1" name="single_room_type[]" onclick="single_room_typer()" value="Single Room">Single Room</label>
</div>
<div class="checkbox">
  <label><input type="checkbox" value="Double Room" id="single_room_type2" onclick="single_room_typer()" name="single_room_type[]">Double Room</label>
</div>
<div class="checkbox">
  <label><input type="checkbox" value="Hall" id="single_room_type3" onclick="single_room_typer()" name="single_room_type[]">Hall</label>
</div>
...
<span id="single_room_typer" style="color:red;"></span>
		...
		<div class="form-group">
				...
				<input type="text" class="form-control" id="car_name" onchange="car_namer();" name="product_nameess" maxlength="150">
				<span id="car_namer" style="color:red;"></span>
				</div>
		<div class="form-group">
				<label>Store Name<b style="color:#FF0000"> *</b></label>
				<input type="text" class="form-control" id="store_name" onchange="store_namer();" name="store_name" maxlength="190">
				<span id="store_namer" style="color:red;"></span>
				</div>
			<div class="form-group">
				<label>Offer Name<b style="color:#FF0000"> *</b></label>
				<input type="text" class="form-control" id="offer_name" onchange="offer_namer();" name="offer_name"  maxlength="190">
				...
				<?php $loan_name=$this->db->query("select offer_name from offer_master order by offer_name asc")->result(); foreach($loan_name as $loan_name1){ ?>
...
<input type="text" id="end_date"   class="form-control" name="end_date"></div>
				</div></div>
		<?php } ?>
				...
				<div  class="form-group" id="loan_typehide" style="display:none;">
				...
				<?php $loan_name=$this->db->query("select loan_name from loan_master order by loan_name asc")->result(); foreach($loan_name as $loan_name1){ ?>
				...
				<div  class="form-group" id="school_name_hide" style="display:none;">
				<label>School Name<b style="color:#FF0000"> *</b> </label>
				<input type="text"  class="form-control" id="school_name" name="school_name" onchange="school_namer();">
				<span id="school_namer" style="color:red;"></span>
				</div>
				
				
				<div  class="form-group" id="bus_route_hide" style="display:none;">
				<label>Route Name<b style="color:#FF0000"> *</b> </label>
				<input type="text"  class="form-control" id="root_num" name="root_num" onchange="root_numr();">
				<span id="root_numr" style="color:red;"></span>
				</div>
				<div  class="form-group" id="hide_school_pattern" style="display:none;">
				...
				<option value="">Select School Pattern</option>
				<?php $loan_name=$this->db->query("select school_pattern from school_pattern order by school_pattern asc")->result(); foreach($loan_name as $loan_name1){ ?>
				...
				<div  class="form-group" id="dept_type_hide" style="display:none;">
				...
				<option value="">Select School Department Type</option>
				<?php $dept_type=$this->db->query("select dept_type from school_dept_type order by dept_type asc")->result(); foreach($dept_type as $loan_name1){ ?>
				...
				<div  class="form-group" id="school_medium_hide" style="display:none;">
				...
				<option value="">Select Medium Of School</option>
				<?php $dept_type=$this->db->query("select school_medium_namee from medium_of_school order by school_medium_namee asc")->result(); foreach($dept_type as $loan_name1){ ?>
				...
				<?php } ?>
				<?php if($category[0]->mid=='4' || $category[0]->mid=='11' || $category[0]->mid=='16') {?>
				...
				<label><?php if($category[0]->mid=='3' || $category[0]->mid=='14' || $category[0]->mid=='13' || $category[0]->mid=='12'){ ?>Price (Optional) <?php }else { ?> Price<b style="color:#FF0000"> *</b> <?php } ?><input type="checkbox" id="negotiable" name="negotiable"   value="Negotiable"> Negotiable</label>
				<input type="text" class="form-control" id="property_price" onchange="property_pricer();" autocomplete="off" onkeypress="return isNumber(event)" name="property_price" />
				<span id="property_pricer" style="color:red;"></span>
				</div>
				<?php } ?>
				...
				<textarea class="form-control" id="ckeditor" name="property_description" placeholder="add your service details here" onchange="ckeditorr();" rows="10" maxlength="500" style="resize:none"></textarea>
				<span id="ckeditorr" style="color:red;"></span>
				</div>
				<div class="form-group">
		 <div class="checkbox">
  <label><input type="checkbox"  onClick="termsandcondtionr()" id="termsandcondtion" name="termsandcondtion" value="">I Agree with <a href="<?php echo site_url(); ?>terms" target="_blank">Terms & Condition</a></label>
</div>

		   <span id="termsandcondtionr" style="color:red;"></span>
        </div>
				...
				<button type="submit" class="btn btn-primary mydbubttt" onClick="return propertypost()">Submit Ad</button> &nbsp;&nbsp;	<img  id="loaderimages" style="display:none;" src="<?php echo base_url(); ?>assets/images/loading.gif" />
...
(function (factory) {
   ...
   // tClose = '<div class="close fileinput-remove">&times;</div>\n';
    ...
    tBtnDefault = '<button type="{type}" tabindex="500" id="removeallimages" title="{title}" class="{css}" {status}>{icon} {label}</button>';
    //noinspection HtmlUnknownAttribute
    tBtnLink = '';
    //noinspection HtmlUnknownAttribute
    ...
    //tActionDelete = '';
    tActionUpload = '';
	<?php //$myid=$this->session->userdata('UID'); $tempdata=$this->db->query("select *from temp_image where  uid='$myid'")->result(); foreach($tempdata as $rows12){ ?>
//	tActionDelete = '<button type="button"  class="kv-file-remove {removeClass}" ' + 'title="{removeTitle}" {dataUrl}{dataKey}>{removeIcon}</button>\n';
	<?php  //} ?>
    ...
    tTagBef1 = tTagBef + '><div class="kv-file-content" style="width: 30px;">\n';
