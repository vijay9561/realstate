  ...
  <div class="row">
    <div class="col-lg-12">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"> <i class="fa fa-home"></i>Advertisement List</li>
      </ol>
...
<?php  $this->session->unset_userdata('updatequery'); } ?>
  ...
  <?php if($this->session->userdata('updatearticals')==1) { ?>
	  <div class="alert alert-success">
  <strong>Success!</strong>Advertisement Update Sucessfully..
</div> 

<?php  $this->session->unset_userdata('updatearticals'); } ?>
<?php if($this->session->userdata('deletearticals')==1) { ?>
	  <div class="alert alert-success">
  <strong>Success!</strong> Advertisement Deleted Sucessfully..
</div> 
...
<?php  $this->session->unset_userdata('deletearticals'); } ?>
  ...
  // $viewdata=$this->db->query("select *from main_category order by mid desc")->result();
   ?>
	...
	<table class="table table-bordered"><thead><tr><th>#</th><th>Advertisement Name</th><th>Advertisement Page</th><th>Advertisement Image</th><th>Created Date</th><th style="width:15%">Action</th></tr></thead>
	<tbody>
	<?php $i=1; 
	 $viewdata=$this->db->query("select *from advertisment order by aid desc")->result();

	foreach($viewdata as $rows){  $days = DateTime::createFromFormat("Y-m-d H:i:s",$rows->date);?>
	<tr>
	<td><?php echo $i++; ?></td>
	<td><?php echo $rows->adveristment_name; ?></td>
	<td><?php echo $rows->top_advertisement; ?></td>
	<td><img src="<?php echo base_url(); ?>adminassets/upload/ad/<?php echo $rows->advertisment_image 	; ?>" style="height:100px;" /></td>
	<td><?php echo $days->format("Y-m-d");?></td>
<td><?php /*?><a href="<?php echo site_url();?>view-main-property-category?action=newsupdat&propertyid=<?php echo  $rows->mid;?>"  title="Edit" class="btn btn-primary"><i class="fa fa-edit"></i></a>&nbsp;<?php */?>
		 <?php if($rows->status=='active') { ?><a href="<?php echo site_url();?>Admin_Controller/updatestatus_advertisment_inactive/<?php echo  $rows->aid; ?>" onclick="return confirm('Are You Sure Update This Status'); " class="btn btn-success"><?php echo $rows->status;  ?></a><?php }else {?><a href="<?php echo site_url();?>Admin_Controller/updatestatus_advertisment_active/<?php echo  $rows->aid; ?>"  onclick="return confirm('Are You Sure Update This Status'); " class="btn btn-danger"><?php echo $rows->status; }  ?></a>&nbsp;
		   <a href="<?php echo site_url(); ?>Admin_Controller/updatestatus_advertisment_delete/<?php echo  $rows->aid;?>" class="btn btn-danger" onClick="return confirm('Are You Sure Delete This Advertisement'); "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
		   </td>
	</tr>
	<?php } ?>
	</tbody>
