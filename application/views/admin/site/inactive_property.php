...
$(document).ready(function(){
  ...
  <div class="row">
	...
	<form name="bulk_action_form" action="<?php echo site_url(); ?>Admin_Controller/inactive_all_property" method="post"/>
	...
	<tbody>
	<?php $i=1; 
	$viewdata=$this->db->query("select *from property_post where status='inactive' and main_categories_id='1' order by pid desc")->result();
...
<a href="<?php echo site_url(); ?>update_admin_property_images/<?php echo $rows->pid; ?>"  title="Update Images" class="btn btn-info"><i class="fa fa-camera"></i></a>-->
		 <?php if($rows->status=='active') { ?><a href="<?php echo site_url();?>Admin_Controller/updatestatus_property_inactive/<?php echo  $rows->pid; ?>" title="active status" onclick="return confirm('Are You Sure Update This Status'); " class="btn btn-success"><?php echo $rows->status;  ?></a><?php }else {?><a title="inactive status" href="<?php echo site_url();?>Admin_Controller/updatestatus_property_inactive_expired/<?php echo  $rows->pid; ?>"  onclick="return confirm('Are You Sure Update This Status'); " class="btn btn-danger"><?php echo $rows->status; }  ?></a>&nbsp;
		   <a href="<?php echo site_url(); ?>Admin_Controller/updatestatus_property_delete_expried/<?php echo  $rows->pid;?>" class="btn btn-danger" onClick="return confirm('Are You Sure Delete This Property Category'); " title="Delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
