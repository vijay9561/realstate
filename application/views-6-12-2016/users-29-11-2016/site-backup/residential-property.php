...
<div class="container">
	<?php if($_GET['propertytype']=='Residential Property') { ?>
	<div class="buy-single">
		<h3>view property in Residential</h3>
		<div class="box-sin">
...
function passwordr(){if($('#password').val()==''){}else{ $('#passwordr').html(' '); }}
  ...
  function forgotpassword(){
	...
	$("#forgootpasswordid").css('display','none');
		...
		<?php } ?>
		<?php if($_GET['propertytype']=='Commercial Property'){ ?>
		
		<div class="buy-single">
		<h3>view property in commercial</h3>
		<div class="box-sin">
			<div class="col-md-12 single-box">
				 <?php
				$propertytype=$_GET['propertytype'];
				 ...
				 if (isset($_GET["page"])) { $page  = $_GET["page"]; } else { $page=1; };
				     $start_from = ($page-1) * $num_rec_per_page;  
		    $MOVIES1=$this->db->query("SELECT * FROM property_post  p inner join property_images pc on p.pid=pc.pid where p.property_residentaily='$propertytype' group by p.pid desc LIMIT $start_from, $num_rec_per_page")->result();
			//echo $this->db->last_query();
  if(!empty($MOVIES1)){    
         foreach($MOVIES1 as $movie){ 
             ...
             ?>
				<div class="box-col" style="box-shadow: 0 2px 5px 0 rgba(0,0,0,.16),0 2px 10px 0 rgba(0,0,0,.12);">
				     ...
				     <div class=" col-sm-7 left-side ">
						<a href="<?php echo site_url(); ?>view_single_property_details/<?php echo $movie->pid; ?>"> <img class="img-responsive" src="<?php echo base_url(); ?>assets/upload/property_image/<?php echo $movie->image_name; ?>" alt=""></a>
					</div>				
				...
				<?php 
				if(!empty($queryall)){
 $queryall=$this->db->query("SELECT * FROM property_post p inner join property_images pc on p.pid=pc.pid where p.property_residentaily='$propertytype' group by p.pid desc")->result();
$total_records = count($queryall);  //count number of records
$total_pages = ceil($total_records / $num_rec_per_page); 
?> 
		<ul class="pagination">
