<div class="container">
	<style>
	.imgclassimages{ height:300px;}
	@media(max-with:500px){
		.imgclassimages{ height:100px;}
	}
	</style>
	...
	<!--	<h3>posted property</h3>-->
		<div class="box-sin">
		    ...
		    $MOVIES1=$this->db->query("SELECT * FROM property_post  p inner join property_images pc on p.pid=pc.pid where  status='active' group by p.pid desc LIMIT $start_from, $num_rec_per_page")->result();
		...
		<ul class="pagination">
				<?php 
 $queryall=$this->db->query("SELECT * FROM property_post p inner join property_images pc on p.pid=pc.pid where  status='active' group by p.pid desc")->result();
...
for ($i=1; $i<=$total_pages; $i++) { ?>
      <li class="page <?php if($_GET['page']==$i){ echo 'active'; } else { echo ''; } ?>"><a href="<?php echo site_url(); ?>view-posted-property?page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
<?php };  ?>
<li class="prev page"><a href="<?php echo site_url(); ?>view-posted-property?page=<?php echo $total_pages;?>"> > </a></li>
</ul></nav></div>
