...
<div class="container">
  ...
  <strong>Success!</strong>Your Property Update Successfully..
</div> 
...
<?php  $this->session->unset_userdata('updateimages'); } ?>
	<!--	<h3>Your posted property List</h3>-->
		<div class="box-sin">
		    ...
		    $MOVIES1=$this->db->query("SELECT * FROM property_post  p inner join property_images pc on p.pid=pc.pid where p.uid='$id' and p.status='active' group by p.pid desc LIMIT $start_from, $num_rec_per_page")->result();
             ...
             ?>
				<div class="box-col" style="box-shadow: 0 2px 5px 0 rgba(0,0,0,.16),0 2px 10px 0 rgba(0,0,0,.12);">
				     ...
				     <div class=" col-sm-6 left-side ">
					  <a  href="<?php echo site_url(); ?>update_your_property/<?php echo $movie->pid;  ?>" class="hvr-sweep-to-right more" >Update Property</a>
					  <a  href="<?php echo site_url(); ?>update_property_images/<?php echo $movie->pid;  ?>" class="hvr-sweep-to-right more" style="float:right" >Update Images</a>
					  <br /><br /> 
					...
					<div class="  col-sm-6 middle-side">
						...
						<?php if($this->session->userdata('UID')){ ?>
									 <tr><th>Your Owner Name:</th><td><?php echo $movie->property_owner_name; ?></td></tr>
 ...
 $queryall=$this->db->query("SELECT * FROM property_post p inner join property_images pc on p.pid=pc.pid where p.uid='$id' and p.status='active' group by p.pid desc")->result();
 //echo $this->db->last_query();
