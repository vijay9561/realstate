...
$(document).ready(function(){
  ...
  <div class="row">
	...
	<input type="submit" class="btn btn-danger" name="bulk_delete_submit"  onclick="return deleteConfirm();" value="Delete" />&nbsp;&nbsp;<input type="submit" value="active" name="bulk_active_submit" class="btn btn-success" onclick="return updatestatus();" />&nbsp;&nbsp;<input type="submit" value="inactive" onclick="return updatestatus();" class="btn btn-danger" name="bulk_inactive_submit" /></th></tr></thead>
	<tbody>
	<?php $i=1; 
	...
	<tr>
	<td><span><?php echo $i++; ?>&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="checked_id[]" class="checkbox" value="<?php echo $rows->pid; ?>"/></span></td>
...
<td>
 &nbsp;&nbsp;
<a href="<?php echo site_url(); ?>view-admin-property-details/<?php echo $rows->pid; ?>"  title="View" class="btn btn-primary"><i class="fa fa-eye"></i></a>&nbsp;
		 <?php if($rows->status=='active') { ?><a href="<?php echo site_url();?>Admin_Controller/updatestatus_property_inactive/<?php echo  $rows->pid; ?>" onclick="return confirm('Are You Sure Update This Status'); " class="btn btn-success"><?php echo $rows->status;  ?></a><?php }else {?><a href="<?php echo site_url();?>Admin_Controller/updatestatus_property_active/<?php echo  $rows->pid; ?>"  onclick="return confirm('Are You Sure Update This Status'); " class="btn btn-danger"><?php echo $rows->status; }  ?></a>&nbsp;
		   <a href="<?php echo site_url(); ?>Admin_Controller/updatestatus_property_delete/<?php echo  $rows->pid;?>" class="btn btn-danger" onClick="return confirm('Are You Sure Delete This Property Category'); "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
